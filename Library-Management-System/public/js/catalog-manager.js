    ...
    return `
      <div class="bg-white dark:bg-slate-900 rounded-lg shadow-md hover:shadow-lg transition-shadow p-4">
        <div class="aspect-square bg-gray-200 dark:bg-slate-800 rounded-lg mb-4 flex items-center justify-center overflow-hidden">
          ${book.Bimage ? `<img src="${book.Bimage}" alt="${book.Bname}" class="w-full h-full object-cover">` : '<span class="material-symbols-outlined text-5xl text-gray-400">menu_book</span>'}
        </div>
        ...
        <div class="flex gap-2">
          ...
          ${isAvailable ? `
            <button onclick="cartService.addItem(${book.Bid}, ${JSON.stringify({Bname: book.Bname, Bprice: book.Bprice, Bimage: book.Bimage})}); showSuccess('Added to cart!');" class="flex-1 bg-green-600 hover:bg-green-700 text-white py-2 px-3 rounded text-sm transition-colors">
              Cart
            </button>
          ` : ''}
        </div>
